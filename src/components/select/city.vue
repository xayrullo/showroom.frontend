...
<template>
  ...
  >
    <option
      v-if="!props.disablePlaceholder && !props.removePlaceholder"
      value=""
    >
      {{ props.placeholder }}
    </option>
    ...
    <option
      ...
      :key="index"
      :value="
        place[valueType] !== '' ? place[valueType] : place.name.substring(0, 3)
      "
      :selected="props.region === place[valueType]"
    >
      {{ props.shortCodeDropdown ? place.shortCode : place.name }}
    </option>
  </select>
</template>
